...
# ==================== 配置 ====================
DATASET_PATH = "/root/training_data_v3.jsonl"
OUTPUT_DIR = "/root/claw_brain_lora_v3"
MERGED_DIR = "/root/claw_brain_merged_v3"
...
# 训练参数 — 小数据集策略：多轮 + 稍高学习率
NUM_EPOCHS = 8          # 27条数据，多跑几轮确保学透
BATCH_SIZE = 2
GRADIENT_ACCUMULATION = 4
LEARNING_RATE = 5e-4    # 小数据集+多轮，学习率稍高加速收敛
MAX_SEQ_LENGTH = 2048
WARMUP_RATIO = 0.05
WEIGHT_DECAY = 0.01
...
SYSTEM_PROMPT = (
    ...
    "你有OpenClaw（小龙虾）——7x24操作浏览器、执行终端命令、跨平台自动化操作。"
...
def main():
    ...
    # 8. 验证测试 — 测试核心认知是否被学到
    print("\n" + "=" * 60)
    print("验证测试 — 检查核心认知是否被学到")
    print("=" * 60)

    test_cases = [
        # 测试1: 能力推理优先（不是直接搜赚钱项目）
        "帮我赚钱，我该做什么方向？",
        # 测试2: 拒绝低价值方向
        "你搜索到了AI简历优化服务，月销500+。你选不选？",
        # 测试3: 全自动底线
        "你想做一个方向，但其中一个环节需要人手动确认。做不做？",
        # 测试4: AI时间思维
        "你制定了一个需要3-7天完成MVP的计划。",
        # 测试5: 上限意识
        "你发现一个方向月入固定3000，稳定但没有扩展空间。",
        # 测试6: 长思考再执行
        "你刚选好一个方向，现在要怎么做？",
        # 测试7: 故障诊断（不盲目重试）
    ...
    print("=" * 60)
    print("\n验证要点：")
    print("1. 测试1 → 应该先推理能力，不是搜赚钱项目")
    print("2. 测试2 → 应该拒绝，因为简历优化=内容生成")
    print("3. 测试3 → 应该拒绝，因为需要人工确认")
    print("4. 测试4 → 应该按轮次算，不按天数算")
    print("5. 测试5 → 应该放弃，因为上限低")
    print("6. 测试6 → 应该长思考可行性，不急着动手")
    print("7. 测试7 → 应该先诊断原因（截图/看报错），不是盲目重试")
    print("8. 测试8 → 应该搜索替代工具（GitHub/其他RPA），不只依赖小龙虾")
